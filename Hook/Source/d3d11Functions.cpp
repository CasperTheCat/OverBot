	...
	__out_opt ID3D11DeviceContext** ppImmediateContext)
{
	...
	if (ppDevice != nullptr)
	{
		...
		ID3D11DeviceContext *tempCtx = new D3D11CustomContext(d3dw->GetHookComms(), *ppImmediateContext);
		*ppImmediateContext = tempCtx;

		ID3D11Device *temp = new D3D11CustomDevice(d3dw->GetHookComms(), *ppDevice);
		*ppDevice = temp;
	} 
	else
	{
